...
## 18. V4.3 增补：防坑体系与门禁（2026-09-13 实战固化 · 33 条 ＋ 三件门禁）

> **定位**：§17 是"工程纪律 9 条"，§18 是"**防坑体系与门禁**"。二者并立、不互相替代；**与 §0–§17 同等强制**。
> **为什么要有它**：2026-09-12～13 两天里，同一个人（AI）在 33 个不同的点上重复踩坑——教训写在哪里都不如写进**执行标准＋可运行门禁**。
...
| 改后 | `python tools\postflight.py` | ①YAML ②desc ≤1024（官方解析器）③表格完整性 ④脚本同步（插件↔权威 4/0/0）⑤**机器层零新增 fail**（对基线 diff）⑥`py_compile` 全脚本 | `0`＝**才算改完**；非 0＝不得宣布完成 |
...
- **收尾必跑**：`postflight.py` **六项全绿**；任一红项不得写"已验证/已交付"。

### 18.3 《蒸馏工程避坑手册》（33 条）

全文（含每条 6 字段：现象／触发场景／立即处置／预防规则／自检命令／证据锚）：
`蒸馏工作区\蒸馏工程避坑手册.md`。**33 条一句话索引**：
...
**A-01…A-10（2026-09-13 新增）**
A-01 "某条线专用"的硬编码路径＝工具报废 ｜ A-02 扫描仪器的"空集陷阱"（只从标记段取词 → 无该段的件永不相配） ｜ A-03 边界声明是**四态**不是两态（含 `共让位→X`） ｜ A-04 判据词表漏一种写法＝整族假警报 ｜ A-05 共享词面有相反语义（共同认领 vs 共同否定） ｜ A-06 **基线/工具副本都必须与正本同路径形状同深度** ｜ A-07 守卫断言必须按**数据形态**分派 ｜ A-08 注释里的"惯例"没登记＝**悬空口径**（等于免检） ｜ A-09 补让位句必须与该件**既有触发词**逐词比对 ｜ A-10 **改权威工具 ⇒ 同批同步副本＋重建打包产物＋双门复验**

### 18.4 闭环规则（防手册腐烂）

**每踩一个新坑，必须同批做两件事**：① 进《蒸馏工程避坑手册》（带**可粘贴的自检命令**与证据锚）；② **可脚本化的同批加进 `tools\postflight.py`**。**只写进聊天或只写进 learnings 不算处理完。**
对账：`pitfall_audit`（设想已写入手册 §7.2）——扫描手册条目，输出"有命令的/没命令的"，并对命令做可执行性抽查。

### 18.5 写入型自检命令禁令（2026-09-13 事故固化成条）
...
《V3.1 全量执行单》"收尾固定项"已增补 **第 34–36 项**：34＝`preflight` 开工前全绿；35＝`postflight` 改后六项全绿；36＝门禁自证 `gate_selftest`（三坏全拦、一好放行）。
...
*蒸馏 V4.3 增补（§18）· 2026-09-13 · **防坑体系与门禁**：三件门禁脚本（preflight／postflight／gate_selftest）＋《蒸馏工程避坑手册》33 条＋闭环规则＋写入型自检禁令。**与 §0–§17 同等强制**。*
